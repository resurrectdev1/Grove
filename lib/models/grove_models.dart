...
class HabitTree {
  ...
  HabitTree({
    ...
    this.excusedDaysCountTowardsStreak = false,
       ...
       _checkInDays = checkInDays != null
           ? Set<DateTime>.of(checkInDays)
           : <DateTime>{},
  ...
  int get peakDays {
    ...
    return _relapses.isEmpty
        ...
        : math.max(
            daysElapsed,
            _relapses.map((e) => e.peakDays).reduce(math.max),
          );
  }
  ...
  factory HabitTree.fromMap(Map<String, dynamic> m) => HabitTree(
    ...
    mode: m['mode'] != null
        ? HabitMode.values[m['mode'] as int]
        : HabitMode.abstain,
    ...
    nullDays: (m['nullDays'] as List<dynamic>?)
        ?.map((d) => DateTime.parse(d as String))
        .toSet(),
    checkInTimestamps: (m['checkInTimestamps'] as Map<String, dynamic>?)?.map(
      (day, ts) => MapEntry(DateTime.parse(day), DateTime.parse(ts as String)),
    ),
    checkInNotes: (m['checkInNotes'] as Map<String, dynamic>?)?.map(
      (day, note) => MapEntry(DateTime.parse(day), note as String),
    ),
    streakFrozen: m['streakFrozen'] as bool? ?? false,
    excusedDaysCountTowardsStreak:
        m['excusedDaysCountTowardsStreak'] as bool? ?? false,
  );
  ...
  HabitTree copyWith({
    ...
    checkInTimestamps:
        checkInTimestamps ?? Map<DateTime, DateTime>.of(_checkInTimestamps),
