...
class FractalTreePainter extends CustomPainter {
  ...
  double get _leafDensity {
    final raw = daysElapsed <= 7
        ? 0.0
        : daysElapsed <= 35
        ? ((daysElapsed - 7) / 28.0).clamp(0.0, 1.0)
        : 1.0;
  ...
  void _drawTree({
    ...
    final trunkLen =
        size.height *
        _lerpD(0.36, 0.58, progress) *
        dna.forkHeight *
        dna.lengthBias;
    final spread =
        _lerpD(0.30, 0.52, progress) *
        dna.spread *
        (1.0 + _genetic('spread') * 0.5);
    final lenRatio =
        _lerpD(0.66, 0.73, progress) *
        dna.lengthBias *
  ...
  void _drawRootFlare(
    ...
    canvas.drawOval(
      ...
      Paint()
        ..color = barkC.withValues(alpha: 0.85)
        ..style = PaintingStyle.fill,
    );
  ...
  void _drawBranch({
    ...
    if (depthFromTip <= 3 && windPhase > 0) {
      ...
      windOffset =
          math.sin(
            windPhase + depthFromTip * 0.9 + _genetic('wind') * math.pi,
          ) *
          windStrength;
    }
    ...
    if (strokeWidth > 1.5) {
      ...
      canvas.drawLine(
        start.translate(
          math.cos(adjustedAngle + math.pi / 2) * strokeWidth * 0.18,
          math.sin(adjustedAngle + math.pi / 2) * strokeWidth * 0.18,
        ),
        tip.translate(
          math.cos(adjustedAngle + math.pi / 2) * tipWidth * 0.10,
    ...
    final childLen =
        length * lengthRatio * (1.0 + _genetic('len_$depth') * 0.4);
    ...
    _drawBranch(
      ...
      depth: depth - 1,
      maxDepth: maxDepth,
      spreadAngle: spreadAngle * 0.90,
      lengthRatio: lengthRatio,
      strokeWidth: childWidth,
      depthFromTip: depthFromTip - 1,
      color: color,
      tipPositions: tipPositions,
    );
    _drawBranch(
    ...
    if (maxDepth >= 4 &&
      ...
      _drawBranch(
        ...
        depth: depth - 2,
        maxDepth: maxDepth,
        spreadAngle: spreadAngle * 0.82,
        lengthRatio: lengthRatio,
        strokeWidth: childWidth * 0.75,
        depthFromTip: depthFromTip - 2,
        color: color,
        tipPositions: tipPositions,
      );
    }
  ...
  void _drawLeafCluster(
    ...
    for (int pass = 0; pass < 2; pass++) {
      ...
      for (int i = 0; i < (isBack ? leafCount ~/ 2 : leafCount); i++) {
        ...
        final leafSize =
            (branchLength * 0.22 +
                    rng.nextDouble() * branchLength * 0.07 +
        ...
        final leafOpacity =
            (isBack ? 0.28 : 0.52 + _leafDensity * 0.22) * distFactor;
        _drawSingleLeaf(
          canvas,
          Offset(lx, ly),
          leafSize * sizeMulti,
  ...
  void _drawSingleLeaf(
    ...
    canvas.drawPath(
      leafPath,
      Paint()
        ..color = leafCol.withValues(alpha: opacity)
        ..style = PaintingStyle.fill,
    );
    ...
    canvas.drawPath(
      highlightPath,
      Paint()
        ..color = highlight.withValues(alpha: opacity * 0.28)
        ..style = PaintingStyle.fill,
    );
  ...
  void _drawSpores(Canvas canvas, Size size, List<Offset> tips) {
    ...
    final breathAlpha =
        ((math.sin(windPhase * 0.4) * 0.5 +
                math.sin(windPhase * 0.17) * 0.15 +
                0.65))
            .clamp(0.25, 1.0);

    final trunkX = size.width * 0.5;
    final groundY = size.height * 0.93;
    final canopyY = tips.fold(
      groundY,
      (double lo, Offset t) => math.min(lo, t.dy),
    );
    ...
    for (int i = 0; i < largeCount; i++) {
      ...
      final fadeOut = ((1.0 - heightRatio) / (1.0 - fadeOutStart)).clamp(
        0.0,
        1.0,
      );
      ...
      final orbitRadius =
          (28.0 + (seedOff % 4) * 10.0) * (1.0 - clampedHR * 0.65);
      final swirl =
          windPhase * (0.55 + (seedOff % 3) * 0.18) +
          seedOff * 0.44 +
          clampedHR * math.pi * 1.8;
      final px = (trunkX + math.cos(swirl) * orbitRadius).clamp(
        4.0,
        size.width - 4.0,
      );
      ...
      for (int t = 3; t >= 1; t--) {
        final tPhase = ((phase - t * 0.055) % (math.pi * 2)).clamp(
          0.0,
          math.pi * 2,
        );
        final tLife = tPhase / (math.pi * 2);
        final tEased = tLife < 0.5
            ? 2 * tLife * tLife
            : 1 - math.pow(-2 * tLife + 2, 2) / 2;
        ...
        final tPx = (trunkX + math.cos(tSwirl) * tOrbit).clamp(
          4.0,
          size.width - 4.0,
        );
    ...
    for (int i = 0; i < dustCount; i++) {
      ...
      final driftX =
          math.sin(phase * 2.7 + seedOff * 0.09) * 22.0 +
  ...
  void _drawMossFragment(
    ...
    for (int b = 0; b < blobCount; b++) {
      ...
      canvas.drawCircle(
        Offset(bx - bRadius * 0.18, by - bRadius * 0.18),
        bRadius * 0.55,
        paint,
      );
    }
  ...
  void _drawSeed(Canvas canvas, Size size) {
    ...
    if (seedMix > 0) {
      ...
      canvas.drawPath(
        seedPath,
        Paint()
          ..color = ac.withValues(alpha: 0.75 * seedMix)
          ..style = PaintingStyle.fill,
      );
      ...
      canvas.drawPath(
        sheenPath,
        Paint()
          ..color = ac.withValues(alpha: 0.30 * seedMix)
          ..style = PaintingStyle.fill,
      );
    ...
    if (sproutMix > 0) {
      ...
      final stemPath = Path()
        ...
        ..quadraticBezierTo(
          stemBase.dx + sproutH * 0.10,
          stemBase.dy - sproutH * 0.5,
          stemTip.dx,
          stemTip.dy,
        );
      canvas.drawPath(
      ...
      final leafGrow = sproutMix > 0.35
          ? ((sproutMix - 0.35) / 0.65).clamp(0.0, 1.0)
          : 0.0;
      if (leafGrow > 0) {
  ...
  bool shouldRepaint(FractalTreePainter old) =>
      ...
      old.daysElapsed != daysElapsed ||
      old.geneticSeed != geneticSeed ||
