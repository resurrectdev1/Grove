...
class CheckInWidgetProvider : AppWidgetProvider() {
    ...
    private fun toggleCheckIn(ctx: Context, widgetId: Int) {
        ...
        if (habitIds.isEmpty()) return
            ...
            val selId = navPrefs.getString("checkin_habit_id_$widgetId", null) ?: return
            ...
            if (mode != 1) return
                ...
                if (isCheckedInToday) {
                    for (day in existing) {
                        if (day != todayKey) newArray.put(day)
                    }
                } else {
                    for (day in existing) newArray.put(day)
    ...
    private fun updateWidget(ctx: Context, mgr: AppWidgetManager, widgetId: Int) {
        ...
        val selId = navPrefs.getString("checkin_habit_id_$widgetId", null)
        ...
        if (habit == null || !isCheckIn) {
            views.setTextViewText(R.id.checkin_habit_name, "Tap to pick a habit")
            views.setTextViewText(R.id.checkin_streak, "—")
            views.setTextViewText(R.id.checkin_icon, "＋")
            views.setInt(R.id.checkin_icon, "setTextColor", 0x884E8B5F.toInt())

            val pickerIntent = makePickerIntent(ctx, widgetId)
            views.setOnClickPendingIntent(R.id.checkin_habit_name, pickerIntent)
            views.setOnClickPendingIntent(R.id.checkin_icon, pickerIntent)

            mgr.updateAppWidget(widgetId, views)
            return
        }

        val habitName = habit.optString("name", "—")
        val streak    = computeCheckInStreak(habit)
        ...
        val color = habit.optInt("color", 0xFF4E8B5F.toInt())
        val iconColor = if (checkedToday) color or (0xFF shl 24) else 0x44E0EBE0.toInt()
        views.setTextViewText(R.id.checkin_icon, if (checkedToday) "✓" else "○")
        views.setInt(R.id.checkin_icon, "setTextColor", iconColor)
        ...
        views.setOnClickPendingIntent(
            R.id.checkin_habit_name,
            makePickerIntent(ctx, widgetId)
        )

        mgr.updateAppWidget(widgetId, views)
    }

    private fun makePickerIntent(ctx: Context, widgetId: Int): PendingIntent {
        val intent = Intent(ctx, HabitPickerActivity::class.java).apply {
            putExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, widgetId)
            data = android.net.Uri.parse("grove://checkin_picker/$widgetId")
