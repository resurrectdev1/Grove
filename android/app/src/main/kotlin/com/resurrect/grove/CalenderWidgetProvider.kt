...
class CalendarWidgetProvider : AppWidgetProvider() {
    ...
    companion object {
        ...
        const val KEY_CAL_HABIT     = "cal_habit_id"
    ...
    override fun onReceive(ctx: Context, intent: Intent) {
        ...
        when (intent.action) {
            ACTION_PREV_MONTH -> shiftMonth(navPrefs, -1)
            ACTION_NEXT_MONTH -> shiftMonth(navPrefs, +1)
            ACTION_PICK_HABIT -> cycleHabit(flutterPrefs, navPrefs)
        }

        // Redraw all calendar widgets
    ...
    private fun cycleHabit(flutterPrefs: SharedPreferences, navPrefs: SharedPreferences) {
        val habits = loadHabitIds(flutterPrefs)
        if (habits.isEmpty()) return
            val currentId = navPrefs.getString(KEY_CAL_HABIT, null)
            val idx       = habits.indexOf(currentId) // returns -1 if not found → next is 0 ✓
            val nextId    = habits[(idx + 1) % habits.size]
            navPrefs.edit().putString(KEY_CAL_HABIT, nextId).apply()
    }
    ...
    private fun updateWidget(ctx: Context, mgr: AppWidgetManager, widgetId: Int) {
        ...
        val selId    = navPrefs.getString(KEY_CAL_HABIT, habitIds.firstOrNull())
        val habit    = selId?.let { loadHabit(flutterPrefs, it) }

        val now   = Calendar.getInstance()
        val year  = navPrefs.getInt(KEY_CAL_YEAR,  now.get(Calendar.YEAR))
        val month = navPrefs.getInt(KEY_CAL_MONTH, now.get(Calendar.MONTH) + 1)
        ...
        // Show arrow hint so user knows it's tappable
        ...
        // Each button gets a unique request code so they never collide
        views.setOnClickPendingIntent(R.id.cal_btn_prev,
                                      makeBroadcast(ctx, ACTION_PREV_MONTH, widgetId * 10 + 1))
        views.setOnClickPendingIntent(R.id.cal_btn_next,
                                      makeBroadcast(ctx, ACTION_NEXT_MONTH, widgetId * 10 + 2))
        views.setOnClickPendingIntent(R.id.cal_habit_name,
                                      makeBroadcast(ctx, ACTION_PICK_HABIT, widgetId * 10 + 3))
    ...
    /**
     * Explicit-component broadcast so Android always routes to this receiver.
     * Without setComponent(), implicit broadcasts are dropped on Android 8+.
     */
    private fun makeBroadcast(ctx: Context, action: String, requestCode: Int): PendingIntent {
    ...
    private fun shiftMonth(prefs: SharedPreferences, delta: Int) {
        val now   = Calendar.getInstance()
        var year  = prefs.getInt(KEY_CAL_YEAR,  now.get(Calendar.YEAR))
        var month = prefs.getInt(KEY_CAL_MONTH, now.get(Calendar.MONTH) + 1)
        month += delta
        if (month < 1)  { month = 12; year-- }
        if (month > 12) { month = 1;  year++ }
        prefs.edit().putInt(KEY_CAL_YEAR, year).putInt(KEY_CAL_MONTH, month).apply()
    }
